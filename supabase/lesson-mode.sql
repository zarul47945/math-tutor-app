...
add column if not exists lesson_mode text not null default 'therapy'
check (lesson_mode in ('therapy', 'consultation'));

drop function if exists public.find_active_session_by_join_code(text);

create function public.find_active_session_by_join_code(input_join_code text)
returns table (
  id uuid,
  title text,
  join_code text,
...
drop function if exists public.get_active_session_room_state(uuid, text);

create function public.get_active_session_room_state(
...
begin
  ...
  return jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'lesson_mode', matched_session.lesson_mode,
